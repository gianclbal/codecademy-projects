...
const handle500Error = (json) => {
  ...
  const changeBootSize = (bootId, action, cb) => {
    ...
    fetch(`/api/v1/boots/${bootId}/size/${action}?${queryString}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json"
      }
    ...
    boots.forEach((boot) => {
      ...
      bootsRow.innerHTML = `
        ...
        <td>
          ...
          <div class="update-size-entry">
                <input type="text" id="new-size" name="new-size">
                <button type="button" id="updateSizeSubmit" onclick="changeBootSize(${boot.id}, 'update-size', renderBootsListCallback(document.getElementById('bootsTableBody')));">Change Size</button>
              </div>
        </td>
      `;
